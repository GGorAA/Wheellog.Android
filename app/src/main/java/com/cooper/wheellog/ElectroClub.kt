...
class ElectroClub {
    ...
    private suspend fun insertEmptyEntryInDb(fileName: String, profileName: String) {
        val trip = WheelLog.db.tripDao().getTripByFileName(fileName)
        if (trip == null) {
            WheelLog.db.tripDao().insert(TripEntity(fileName = fileName, profileName = profileName))
        }
    }

    suspend fun updateEntryInDb(track: JSONObject, fileName: String): TripEntity {
        // gets or create trip with fileName field
        val trip = WheelLog.db.tripDao().getTripByFileName(fileName)
                ?: TripEntity(fileName = fileName)
        trip.apply {
            ecId = track.getInt("id")
            startTime = track.optInt("start_time")
            transportId = track.optInt("garage_id")
            urlImage = track.optString("image")
            urlImage2 = track.optString("stats_image")
            duration = track.optInt("duration")
        }
        WheelLog.db.tripDao().update(trip)
        return trip
    }
